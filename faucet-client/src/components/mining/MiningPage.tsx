    ...
    if(this.state.loadingError) {
      return (
        <div className='alert alert-danger'>
          {embedMode ? "Can't claw for this session" : "Can't mine for this session"}: {typeof this.state.loadingError == "string" ? this.state.loadingError : ""}<br />
        ...
        <div className="faucet-actions center">
          <button 
            ...
            disabled={!this.state.clientConnected || this.state.closingSession}>
              {this.state.isClaimable ? "Stop Clawing & Claim Rewards" : "Stop Clawing"}
          </button>
          </div>
      </div>
    );
  ...
  private async onStopMiningClick(force?: boolean) {
    if(!this.state.isClaimable && this.powSession.getBalance() > 0n && !force) {
      this.props.pageContext.showDialog({
        title: "Clawing balance too low",
        body: (
          <div className='alert alert-warning'>
            Your clawing balance of {toReadableAmount(this.powSession.getBalance(), this.props.faucetConfig.faucetCoinDecimals, this.props.faucetConfig.faucetCoinSymbol)} is too low to be claimed.<br />
            ...
            Do you want to stop clawing and lose the rewards you've already collected?
            </div>
        ),
        closeButton: {
          caption: "Continue clawing",
        },
        applyButton: {
          caption: "Stop clawing",
          applyFn: () => {
